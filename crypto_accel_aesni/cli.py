"""
Command-Line Interface for AES-NI & ARMv8-CE Hardware Cryptographic Acceleration Auditor.
"""
import argparse
import csv
import json
...
from .agents import HardwareCryptoCoordinator

coordinator = HardwareCryptoCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="crypto-hardware-accel-aesni-agent", description="AES-NI & ARMv8-CE Hardware Cryptographic Acceleration Auditor")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  AES-NI & ARMV8-CE HARDWARE CRYPTOGRAPHIC ACCELERATION AUDITOR")
        print(f"  Domain: Cryptographic Engineering | Standard: NIST SP 800-38D (AES-GCM)")
    ...
    if args.command == "batch":
        ...
        if not out_rows:
            print("Error: No valid records to process.", file=sys.stderr)
            return 1
        ...
        print(f"Processed {len(out_rows)} records -> {args.output}" + (f" ({errors} skipped)" if errors else ""))
        return 0
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting AES-NI & ARMv8-CE Hardware Cryptographic Acceleration Auditor on http://{args.host}:{args.port}")
